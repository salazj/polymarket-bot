"""
Polymarket CLOB WebSocket client for real-time orderbook and trade data.
...
class PolymarketWSClient:
    """
    WebSocket client with reconnection, heartbeat monitoring, and
    message dispatch for orderbook/trade updates.
    """
    ...
    def subscribe_book(self, token_ids: list[str]) -> None:
        """Queue a book subscription (sent on connect/reconnect)."""
        for token_id in token_ids:
            ...
            # Prevent duplicate subscriptions on repeated calls
            if sub not in self._subscriptions:
                self._subscriptions.append(sub)

    def subscribe_trades(self, token_ids: list[str]) -> None:
        for token_id in token_ids:
            sub = {
                "type": "subscribe",
                "channel": "trades",
    ...
    def subscribe_user(self) -> None:
        """Subscribe to user-specific order/fill updates (requires auth on some APIs)."""
    ...
    async def _dispatch(self, msg: dict[str, Any]) -> None:
        """Route a message to registered handlers by type/channel."""
